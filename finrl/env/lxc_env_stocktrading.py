...
class lxcStockTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        #print('self.df.index.unique() is:', self.df.index.unique())
        #print('self.terminal is:',self.terminal)
        ...
        if self.terminal:
            ...
            #print('in actions is:',actions)
            #print(self.lxcModels is None)
            #test_env, test_obs = lxcStockTradingEnv.get_sb_env(self)
            lxc_actions=[]
            #print('self.state is:',self.state)
            for oneMode in self.lxcModels:
                temp,_=oneMode.predict(self.state)
                lxc_actions.append(temp)
            #print(self.lxcModels)
            temp_action = actions[0]
            if (actions[0] >= 0.5):
                actions = lxc_actions[0]
            elif actions[0] >= -0.5:
                actions = lxc_actions[1]
            else:
                actions = lxc_actions[2]
            ...
            #print('lxc action123 is:',actions)
            #print('sell_index is :',sell_index)
            ...
            if (self.day-1)%2==0:
                if(temp_action>0.5):
                    print('A2C model is:',temp_action,'reward is:',self.reward)
                elif temp_action>-0.5:
                    print('SAC model is:', temp_action, 'reward is:', self.reward)
                else:
                    print('DDPG model is:',temp_action,'reward is:',self.reward)
