...
class StockTradingEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.normal_high = np.zeros(state_space)+0.1
        self.normal_low = np.zeros(state_space)-0.1
        self.state_space = state_space
        ...
        self.trades = 0
        self.episode = 0
        self.all_action = [0 for x in range(stock_dim)]
    ...
    def normalization(self):
        ...
        for i in range(0,temp):
            if (self.state[i] > self.normal_high[i]):
                data.append(1)
                self.normal_high[i] = self.state[i]
            elif (self.state[i] < self.normal_low[i] or self.normal_low[i]==-0.1):
                data.append(0)
                self.normal_low[i] = self.state[i]
            else:
                data.append(((self.state[i] - self.normal_low[i]) / (self.normal_high[i] - self.normal_low[i])))

        for i in range(temp,len(self.state)):
            data.append(self.state[i])
        #print("after self.state:", self.state[0],"self.normal_high[0]: ",self.normal_high[0],"self.normal_low[0]:",self.normal_low[0] )
        #print("after self.state:", data[0])
        #print("data is:", data)
        return data
    ...
    def _sell_stock(self, index, action):
        def _do_sell_normal():
            if self.state[index + 1] > 0:
                ...
                if self.state[index + self.stock_dim + 1] > 0:
                    #print("self.state[index + self.stock_dim + 1]: ", self.state[index + self.stock_dim + 1])
                    # Sell only if current asset is > 0
                    sell_num_shares = min(abs(action), self.state[index + self.stock_dim + 1])
                    sell_amount = self.state[index + 1] * sell_num_shares * (1 - self.sell_cost_pct)
                    # update balance
                    self.state[0] += sell_amount
                    #print("11111self.state[index + self.stock_dim + 1]",self.state[index + self.stock_dim + 1])
                    #print("index + self.stock_dim + 1 is: ",index + self.stock_dim + 1)
                    self.state[index + self.stock_dim + 1] -= sell_num_shares
                    #print('sell_num_shares is:', sell_num_shares)
                    #print("self.state is:",self.state)
                    #print("22222self.state[index + self.stock_dim + 1]", self.state[index + self.stock_dim + 1])
        ...
        def _lxc_do_sell_normal():
            if self.state[index + 1] > 0:
                ...
                if self.state[index + self.stock_dim + 1] > 0:
                    ...
                    sell_num_shares = min(abs(action), self.state[index + self.stock_dim + 1])
                    sell_amount = self.state[index + 1] * sell_num_shares * (1 - self.sell_cost_pct)
                    sell_amount = self.data.open * sell_num_shares * (1 - self.sell_cost_pct)
                    # update balance
                    self.state[0] += sell_amount
                    #print("11111self.state[index + self.stock_dim + 1]",self.state[index + self.stock_dim + 1])
                    #print("index + self.stock_dim + 1 is: ",index + self.stock_dim + 1)
        ...
        if self.turbulence_threshold is not None:
            if self.turbulence >= self.turbulence_threshold:
                print('turbulence_threshold bigger')
                if self.state[index + 1] > 0:
                    # Sell only if the price is > 0 (no missing data in this particular date)
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.all_action = [self.all_action[i] + actions[i] for i in range(min(len(self.all_action), len(actions)))]
        ...
        return self.state, self.reward, self.terminal,{"actions":actions}
